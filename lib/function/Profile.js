...
export const queryProfileSnap = async (DB, profile) => {
	...
	catch(error) { throw LogError('fatal', '主线', '查询~[档案快照]', `✖ ~[名称]~{${profile.ProfileName}} ~[id]~{${profile.ProfileID}}`, error); }
};
export const createProfileSnap = async (DB, profile) => {
	...
	catch(error) { throw LogError('fatal', '主线', `创建~[档案快照]`, `✖ ~[名称]~{${profile.ProfileName}} ~[id]~{${profile.ProfileID}}`, error); }
};
...
export const fetchProfileID = async (userName) => {
	...
	catch(error) { throw LogError('fatal', '主线', `获取~[映射]`, `✖ ~[档案名称]~{${userName}}`, error); }
};
export const fetchProfile = async (idProfile) => {
	...
	catch(error) { throw LogError('fatal', '主线', `获取~[档案]`, `✖ ~[ID]~{${idProfile}}`, error); }
};