...
export const queryProfileSnap = async (DB, profile) => {
	try {
		G.warnU('主线', '查询~[档案快照]', `? ~[名称]~{${profile.ProfileName}}`);
		...
		if(snapProfile) {
			...
			G.warnD('主线', '查询~[档案快照]', `不存在 ~[名称]~{${profile.ProfileName}}`);
		}

		return snapProfile;
	}
	catch(error) { throw LogError('fatal', '主线', '查询~[档案快照]', `✖ `, error); }
};
export const createProfileSnap = async (DB, profile) => {
	...
	catch(error) { throw LogError('fatal', '主线', `创建~[档案快照]`, `✖ `, error); }
};
