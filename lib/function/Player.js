...
export const queryPlayer = async (DB, nick) => {
	...
	catch(error) { throw LogError('fatal', '主线', '查询~[玩家]', `✖ ~[昵称]~{${nick}}`, error); }
};
export const createPlayer = async (DB, nick) => {
	...
	catch(error) { throw LogError('fatal', '主线', `创建~[玩家]`, `✖ ~[昵称]~{${nick}}`, error); }
};
...
export const queryPlayerProfiles = async (DB) => {
	...
	catch(error) { throw LogError('fatal', '主线', `查询所有~[玩家]`, `✖ `, error); }
};