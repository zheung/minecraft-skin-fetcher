...
export const querySkin = async (DB, hashSkin) => {
	...
	catch(error) { throw LogError('fatal', '主线', '查询~[皮肤]', `✖ ~[哈希]~{${hashSkin}}`, error); }
};
export const createSkin = async (DB, hashSkin, bufferSkin) => {
	...
	catch(error) { throw LogError('fatal', '主线', `创建~[皮肤]`, `✖ ~[哈希]~{${hashSkin}}`, error); }
};
...
export const fetchSkin = async (hashSkin, urlSkin) => {
	...
	catch(error) { throw LogError('fatal', '主线', `获取~[皮肤]`, `✖ ~[哈希]~{${hashSkin}}`, error); }
};