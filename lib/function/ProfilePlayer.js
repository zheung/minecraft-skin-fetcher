...
export const queryPlayerProfile = async (DB, idProfile) => {
	...
	catch(error) { throw LogError('fatal', '主线', '查询~[玩家档案]', `✖ `, error); }
};
export const createPlayerProfile = async (DB, player, idProfile) => {
	try {
		G.warnU('主线', `更新~[玩家档案]`, `? ~[档案ID]~{${idProfile}}`);

		const profilePlayer = await DB.query('INSERT INTO "PlayerProfile"$i RETURNING *', { PlayerID: player.id, ProfileID: idProfile });

		G.infoD('主线', `更新~[玩家档案]`, `✔ ~[档案ID]~{${idProfile}}`);

		return profilePlayer;
	}
	catch(error) { throw LogError('fatal', '主线', `更新~[玩家档案]`, `✖ ~[档案ID]~{${idProfile}}`, error); }
};
